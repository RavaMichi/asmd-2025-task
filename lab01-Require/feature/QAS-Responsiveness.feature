...
Feature: Responsiveness
    ...
    Scenario Outline: Single request under heavy load is handled in < 1s
        Given the server is under heavy load of <num_requests> concurrent connections for <load_duration> seconds
        ...
        And the measured response time for that request is less than "5" second
        Examples:
            | num_requests | load_duration | method | endpoint                      |
